...
// In the update quantity section, add validation:
...
// In the remove from cart section:
...
$query = $conn->prepare("SELECT p.product_name, p.product_price, c.quantity, p.product_id
                         FROM cart_items c 
                         JOIN products p ON c.product_id = p.product_id 
                         WHERE c.user_id = ?");
$query->bind_param("i", $user_id);
$query->execute();
$query->store_result(); // Store the result for row count

if ($query->num_rows > 0) {  // Check if there are rows
    echo '<table>';
